...
static bool pdb_replace(const std::string& fname, const std::string& pdb_name, char* pdb_name_buffer, int pdb_name_len) {
    HANDLE fp = nullptr;
    ...
    do {
        ...
        for (int i = 1; i <= numEntries; i++, debugDir++) {
            ...
            if (pdb_found && strlen(pdb_found) >= strlen(pdb_name.c_str())) {
...
bool ProcessPDB(const std::string& fname, const std::string& pdbname) {
    char ORIG_PDB_FILE[MAX_PATH];
