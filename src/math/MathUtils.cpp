...
namespace vpsk {
    ...
    float PointOnEllipseBisector(const size_t & num_components, const glm::vec2 & extents, const glm::vec2 & y,
         glm::vec2 & x) {
        ...
        float e_min = extents[static_cast<glm::length_t>(num_components - 1)];
        ...
        const glm::length_t j_max = static_cast<glm::length_t>(std::numeric_limits<float>::digits - std::numeric_limits<float>::min_exponent);
        float s_min = z[static_cast<glm::length_t>(num_components - 1)];
        float s_max;
        ...
        float s = 0.0f;
        for (glm::length_t j = 0; j < j_max; ++j) {
            s = (s_min + s_max) * 0.50f;
            if (s == s_min || s == s_max) {
                break;
            }
    ...
    float PointOnEllipseSqrDistanceImpl(const glm::vec2 & extents, const glm::vec2 & y, glm::vec2 & x) {
        ...
        if (y[1] > 0.0f) {
            ...
            for (glm::length_t i = 0; i < num_pos; ++i) {
        ...
        for (glm::length_t i = 0, num_pos = 0; i < 2; ++i) {
            if (y[i] > 0.0f) {
                x[i] = x_pos[num_pos];
                ++num_pos;
            }
    ...
    glm::vec2 GetClosestPointOnEllipse(const glm::vec2 & center, const glm::vec2 & axis_a, 
        const glm::vec2 & axis_b, const glm::vec2 & test_point) {
