...
static std::vector<unsigned short> inputCharactersBuffer;
static std::array<bool, 512> keys;
static std::array<GLFWcursor*, 8> standardCursors;
...
void ScrollCallback(double x_offset, double y_offset) {
    MouseScrollX += x_offset;
    MouseScrollY += y_offset;
}

void CharCallback(unsigned int code_point) {
    if (code_point > 0 && code_point < 0x10000) {
        inputCharactersBuffer.emplace_back(static_cast<unsigned short>(code_point));
    }
}
...
void ImGuiWrapper::newFrame() {
    ...
    io.MouseWheelH = MouseScrollX;
    io.MouseWheel = MouseScrollY;
    ...
    for (auto& ch : inputCharactersBuffer) {
...
void ImGuiWrapper::endFrame() {
    inputCharactersBuffer.clear();
    inputCharactersBuffer.shrink_to_fit();
}
...
void ImGuiWrapper::createFontSampler() {
    ...
    fontSampler = resourceContext->CreateSampler(&sampler_info, nullptr);
}
...
void ImGuiWrapper::createShaders() {
    vert = std::make_unique<vpr::ShaderModule>(device->vkHandle(), VK_SHADER_STAGE_VERTEX_BIT, __glsl_shader_vert_spv, sizeof(__glsl_shader_vert_spv));
    frag = std::make_unique<vpr::ShaderModule>(device->vkHandle(), VK_SHADER_STAGE_FRAGMENT_BIT, __glsl_shader_frag_spv, sizeof(__glsl_shader_frag_spv));
}
